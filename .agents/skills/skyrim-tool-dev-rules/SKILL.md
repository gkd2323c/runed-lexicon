...
4. **同一大文件的批量查询 / 扫描必须单次调用覆盖多模式**：多形态复扫用 `query.py --dst "(a|b|c)" --regex` 一次扫完，禁止逐词循环（N 次全文件重扫；实测 30 词逐扫约 30s vs 单次 0.36s）。互相独立的批处理可进程级并行（如多批 consume / apply_fixes）；写同一目标文件（canonical）的操作必须保持单进程。
