...
```json
{
  ...
  "residual_risks": ["xml_index 97 说话人推测为守卫，无结构证据"]
}
```

## 字段约定

- `artifacts`：本节点产出的文件（相对项目根）。Hanako 会 stat 确认存在。
...
Hanako 打回补交。summary 与 handoff 都写：summary 一句话说人话，
handoff 给结构化证据。
