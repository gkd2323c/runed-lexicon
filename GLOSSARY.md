...
以下词条来自对已完成 MOD（MVF1 / SB1 / dg04 / meresis / Artaeum）术语决策的归纳，逐条经 `dictionary/` 官方语料核验；仅收录官方未稳定覆盖、存在跨 MOD 复用必要、或存在称谓边界需要项目统一约束的决策。官方 Skyrim / DLC 已稳定固定且无歧义冲突的译名不在此列（直接查 `dictionary/`）。
...
| Aldmeri Dominion                  | 先祖神洲                            | 势力               | CONFIRMED   | 官方既有（Dawnguard/Update/CC 词典书籍正文，如“萨蒂亚向先祖神洲出卖自己的人民”Update、“帝国和先祖神洲的很多贵族”Dawnguard；字形洲为官方用字）；GLOBAL §4.1 | 梭默背后的高精灵政权；与 Thalmor（执法机关）分层；裸词 the Dominion 明确指代时简称神洲；词典 lookup 需 --contains（专名嵌于整句源，exact 搜不到）；禁音译多米尼恩 |
...
| Dunmer / Dark Elf                 | 丹莫 / 暗精灵                       | 种族称谓           | CONFIRMED   | 官方 RACE；GLOBAL §6                                                                               | 源文字面强制映射：Dunmer=丹莫、Dark Elf=暗精灵；同体系 Altmer=傲特莫、High Elf=高精灵、Bosmer=波斯莫、Wood Elf=木精灵。取消语境弹性，源文写哪个词形就用对应译名，不得按语境互换（高等精灵官方零出现） |
...
| Valenwood                         | 威伦森林                            | 地名               | PROVISIONAL | TES 既有省份；官方词典零见证；Artaeum 定案（全库 11:9 二形，取多数形）                             | 泰姆瑞尔西南大省。**禁“威木”**（Artaeum 的 AuridonSynodQ01/Q02 任务线曾用，已回正）。官方未见稳定形，后续 MOD 沿用“威伦森林”；若官方语料出现异形，按“官方优先”修订本条目 |
