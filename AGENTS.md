...
规则扫描（gate / noun-audit / noun-consistency-scan）只覆盖已登记词与已定义模式，规则零命中不等于收敛；对白称呼、未登记词族、词根家族发散的漂移只有开集发现能看见。名词收敛核查必须叠加启发式种子扩散（分块读译文找专名种子 → 同英文锚全文扩散 → 中文形态归组 → 主会话裁决），发现器不带正确性知识、只报候选，裁决由主会话按 EDID/语境/官方证据执行。声明"名词收敛/实体收敛"时，规则扫描 + 启发式扫描缺一不可。细节见 `noun-consistency-scan` 与 `hana-subagent-ops` SKILL。
